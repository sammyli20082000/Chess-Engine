...
public class GameAreaPanel extends JPanel {
	...
	private double boardImageTangent = 1.0, gamePanelTangent = 1.0; // height /
																	// width,
																	// that is
																	// (y/x)
	private double mouseXTang, mouseYTang; // relative coordinate not pixel;
											// range from 0 to 1
	...
	private void setupBoardImage() {
		try {
			...
			e.printStackTrace();
			boardImageTangent = 1.0 * DataAndSetting.BoardData.preferredPixelHeight
					/ DataAndSetting.BoardData.preferredPixelWidth;
			calcGamePanelPreferredSize(DataAndSetting.BoardData.preferredPixelHeight,
					DataAndSetting.BoardData.preferredPixelWidth);
		}
	}
	...
	private void calcGamePanelPreferredSize(int width, int height) {
		...
		Boolean boardImageWidthExceedScreenWidth = width > monitorResolution.getWidth() * screenBound,
	...
	public void paintComponent(Graphics g) {
		...
		if (board != null) {
			if (ui.getIsShowPiecePlacingPoint())
				drawAllPiecePlacingPointsForDebug(g);
			drawCurrentPieces(g);
			drawFrameForSelectedObject(g);
			drawPieceNextMoveCandidatePoints(g);
		}

		if (ui.getIsShowDebug())
			printDebugLog(g);
	}
	...
	private void drawFrameForSelectedObject(Graphics g) {
		...
		double posX = selectedPoint.getPosX(), posY = selectedPoint.getPosY(),
		...
		for (int i = 0; i < 2; i++) {
			for (int j = 0; j < 2; j++) {
				int w = (int) (width * boardPaintWidth * frameLength),
						h = (int) (height * boardPaintHeight * frameThickness);

				g.fillRect(
						(int) ((posX - width / 2.0 + width * (1 - frameLength) * j) * boardPaintWidth
								+ boardBaseXShift),
						(int) ((posY - height / 2.0 + height * (1 - frameThickness) * i) * boardPaintHeight
								+ boardBaseYShift),
						(w > 0) ? w : 1, (h > 0) ? h : 1);
				...
				g.fillRect(
						(int) ((posX - width / 2.0 + width * (1 - frameThickness) * j) * boardPaintWidth
								+ boardBaseXShift),
						(int) ((posY - height / 2.0 + height * (1 - frameLength) * i) * boardPaintHeight
								+ boardBaseYShift),
						(w > 0) ? w : 1, (h > 0) ? h : 1
	...
	private void printDebugLog(Graphics g) {
		...
		printString(g, "selected Point: ", new double[] { ui.getCallback().getSelectedPointOrPiece() == null ? -1
				: ui.getCallback().getSelectedPointOrPiece().getId() });
	}
	...
	private void drawCurrentPieces(Graphics g) {
		for (Point point : board.getPoints()) {
			...
			if (piece != null) {
				double pieceWidth = piece.getWidth(), pieceHeight = piece.getHeight();

				g.drawImage(piece.getPieceImage(),
						...
						(int) (pieceWidth * boardPaintWidth), (int) (pieceHeight * boardPaintHeight), null);
			}
		}
	}
	...
	private void drawPiecePlacingPoints(Graphics g, ArrayList<Point> points, Color color, boolean showID) {
		for (Point p : points) {
			if (p != null) {
				int w = (int) (p.getWidth() * boardPaintWidth), h = (int) (p.getHeight() * boardPaintHeight),
	...
	private Point checkThatPointOrPieceClicked() {
		...
		for (Point p : board.getPoints()) {
			...
			if ((pieceOnPoint != null && p.getPosX() - pieceOnPoint.getWidth() * 0.5 <= mouseXTang
					&& p.getPosX() + pieceOnPoint.getWidth() * 0.5 >= mouseXTang
					&& p.getPosY() - pieceOnPoint.getHeight() * 0.5 <= mouseYTang
					&& p.getPosY() + pieceOnPoint.getHeight() * 0.5 >= mouseYTang)
					|| (p.getPosX() - p.getWidth() * 0.5 <= mouseXTang && p.getPosX() + p.getWidth() * 0.5 >= mouseXTang
